...
abstract class SingleAbstractMethodLowering(val context: CommonBackendContext) : FileLoweringPass, IrElementTransformerVoidWithContext() {
    ...
    private fun createObjectProxy(superType: IrType, wrapperVisibility: Visibility, createFor: IrElement): IrClass {
        ...
        val hasExtensionReceiverInt = if (superMethod.extensionReceiverParameter == null) 0 else 1
        val wrappedFunctionClass =
            if (superMethod.isSuspend)
                context.ir.symbols.suspendFunctionN(superMethod.valueParameters.size + hasExtensionReceiverInt).owner
            else
                context.ir.symbols.functionN(superMethod.valueParameters.size + hasExtensionReceiverInt).owner
        ...
        subclass.addFunction {
            ...
            body = context.createIrBuilder(symbol).irBlockBody {
                +irReturn(irCall(wrappedFunctionClass.functions.single { it.name == OperatorNameConventions.INVOKE }).apply {
                    ...
                    valueParameters.forEachIndexed { i, parameter -> putValueArgument(hasExtensionReceiverInt + i, irGet(parameter)) }
                })
            }
        }
